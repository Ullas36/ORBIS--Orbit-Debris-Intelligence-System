...
def load_data():
    ...
    for label, url in SOURCES.items():
        try:
            r = requests.get(url, timeout=30, headers={"User-Agent":"ORBIS/1.0"})
...
if df.empty or "risk_tier" not in df.columns:
    st.error("Could not load debris catalog. CelesTrak may be temporarily unavailable.")
    ...
    with tab1:
        ...
        fig.update_layout(height=600,
            scene=dict(bgcolor="rgba(2,6,15,1)",xaxis_title="X (km)",yaxis_title="Y (km)",zaxis_title="Z (km)"),
    ...
    with tab2:
        ...
        fig2=px.histogram(df[df["altitude_km"]<3000],x="altitude_km",color="object_type",nbins=80,
            ...
            color_discrete_map={"Rocket Body":"#E24B4A","Debris Fragment":"#EF9F27","Defunct Satellite":"#378ADD","Active/Unknown":"#888780"})
