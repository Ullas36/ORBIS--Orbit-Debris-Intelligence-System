...
def load_data():
    # Try multiple CelesTrak endpoints
    GROUPS = [
        "cosmos-deb", "iridium-33-deb", "fengyun-1c-deb",
        "rocket-bodies", "active"
    ]
    BASE_TLE  = "https://celestrak.org/NORAD/elements/gp.php"
    BASE_JSON = "https://celestrak.org/NORAD/elements/gp.php"
    ...
    for group in GROUPS:
        ...
        # Try 1: JSON format (more reliable from cloud)
        try:
            url = f"{BASE_JSON}?GROUP={group}&FORMAT=json"
            r = requests.get(url, timeout=30,
                headers={"User-Agent": "ORBIS-Research/1.0 (academic project)"})
            if r.status_code == 200 and r.text.strip().startswith("["):
                data = r.json()
                for obj in data:
                    try:
                        name = obj.get("OBJECT_NAME","UNKNOWN")
                        nid  = int(obj.get("NORAD_CAT_ID", 0))
                        if nid == 0 or nid in all_tles:
                            continue
                        # Reconstruct TLE lines from JSON fields
                        tle1 = obj.get("TLE_LINE1","")
                        tle2 = obj.get("TLE_LINE2","")
                        if tle1 and tle2:
                            all_tles[nid] = (name, tle1, tle2)
                    except: continue
                fetched = True
        except: pass

        # Try 2: TLE text format fallback
        if not fetched:
            try:
                url = f"{BASE_TLE}?GROUP={group}&FORMAT=tle"
                r = requests.get(url, timeout=30,
                    headers={"User-Agent": "ORBIS-Research/1.0 (academic project)"})
    ...
    # Try 3: Space-Track mirror via celestrak if still empty
    if len(all_tles) < 10:
        FALLBACK_URLS = [
            "https://celestrak.org/NORAD/elements/gp.php?GROUP=cosmos-deb&FORMAT=tle",
            "https://celestrak.org/NORAD/elements/gp.php?GROUP=iridium-33-deb&FORMAT=tle",
        ]
...
with st.spinner("Loading live debris catalog from CelesTrak..."):
    df = load_data()

if df.empty or "risk_tier" not in df.columns:
    st.error("Could not load debris catalog from CelesTrak.")
    st.warning("CelesTrak occasionally blocks cloud server IPs. Try clicking Refresh TLE data.")
    st.info("While waiting, here are the key project results:")
    col1,col2,col3=st.columns(3)
    col1.metric("ML Accuracy","98.8%","Random Forest classifier")
    col2.metric("DeltaVNet MAPE","3.82%","vs Hohmann formula")
    col3.metric("QML Speedup","28x faster","at 5,000 pairs")
else:
